import { PrismaClient, UserRole, PropertyType, UnitStatus, LeaseStatus, ChargeStatus, PaymentProvider, PaymentMatchStatus, LateFeeType } from '@prisma/client';
...
async function main() {
  ...
  await prisma.user.upsert({
    ...
    update: {},
    create: { email: 'admin@rentflow.ke', phone: '+254700000001', passwordHash: superAdminHash, firstName: 'System', lastName: 'Admin', role: UserRole.SUPER_ADMIN },
  });
  ...
  const landlordUser = await prisma.user.upsert({
    ...
    update: {},
    create: { email: 'james.kamau@gmail.com', phone: '+254711222333', passwordHash: landlordHash, firstName: 'James', lastName: 'Kamau', role: UserRole.LANDLORD },
  });
  ...
  await prisma.user.upsert({
    ...
    update: {},
    create: { email: 'peter.odhiambo@kamauprops.ke', phone: '+254722333444', passwordHash: managerHash, firstName: 'Peter', lastName: 'Odhiambo', role: UserRole.MANAGER, landlordId: landlord.id },
  });
  ...
  const propA = await prisma.property.upsert({
    ...
    update: {},
    create: { landlordId: landlord.id, name: 'Kileleshwa Heights', code: 'KILELESHWA', address: 'Kileleshwa Road, Westlands', county: 'Nairobi', propertyType: PropertyType.APARTMENT_BLOCK },
  });

  const propB = await prisma.property.upsert({
    where: { landlordId_code: { landlordId: landlord.id, code: 'RONGAI' } },
    update: {},
    create: { landlordId: landlord.id, name: 'Rongai Gardens', code: 'RONGAI', address: 'Rongai Town, Kajiado', county: 'Kajiado', propertyType: PropertyType.BUNGALOW },
  });

  // Units
  const unitData = [
    { propertyId: propA.id, unitNumber: 'A1', floor: '1st', rentAmount: 25000, status: UnitStatus.OCCUPIED },
    { propertyId: propA.id, unitNumber: 'A2', floor: '1st', rentAmount: 25000, status: UnitStatus.OCCUPIED },
    { propertyId: propA.id, unitNumber: 'B1', floor: '2nd', rentAmount: 28000, status: UnitStatus.OCCUPIED },
    { propertyId: propA.id, unitNumber: 'B2', floor: '2nd', rentAmount: 28000, status: UnitStatus.VACANT },
    { propertyId: propB.id, unitNumber: 'H1', floor: 'Ground', rentAmount: 15000, status: UnitStatus.OCCUPIED },
    { propertyId: propB.id, unitNumber: 'H2', floor: 'Ground', rentAmount: 15000, status: UnitStatus.OCCUPIED },
  ];
  const units = [];
  ...
  for (const td of tenantData) {
    ...
    const user = await prisma.user.upsert({
      ...
      update: {},
      create: { email: td.email, phone: td.phone, passwordHash: hash, firstName: td.firstName, lastName: td.lastName, role: UserRole.TENANT, landlordId: landlord.id },
    });
  ...
  await prisma.lateFeeRule.upsert({
    ...
    update: {},
    create: { id: 'default-late-fee', landlordId: landlord.id, name: 'Standard Late Fee', gracePeriodDays: 5, feeType: LateFeeType.FIXED, feeValue: new Decimal(500), isRecurring: false, isActive: true },
  });
  ...
  for (const lc of leaseConfigs) {
    const lease = await prisma.lease.upsert({
      ...
      update: {},
      create: { landlordId: landlord.id, tenantId: lc.tenant.id, unitId: lc.unit.id, accountReference: lc.ref, monthlyRent: new Decimal(lc.rent), depositAmount: new Decimal(lc.rent), depositPaid: true, paymentDueDay: 1, startDate: new Date('2024-01-01'), status: LeaseStatus.ACTIVE },
    });
    for (let i = 2; i >= 0; i--) {
      ...
      if (!existing) {
        await prisma.rentCharge.create({
          data: { leaseId: lease.id, tenantId: lc.tenant.id, landlordId: landlord.id, amountDue: new Decimal(lc.rent), amountPaid: new Decimal(0), balance: new Decimal(lc.rent), billingPeriod, dueDate, status: ChargeStatus.UNPAID },
        });
      }
    }
  }

  // Sample payments for Grace (all 3 months paid)
  ...
  for (let i = 0; i < graceCharges.length; i++) {
    ...
    if (!existing) {
      const payment = await prisma.payment.create({
        data: { landlordId: landlord.id, tenantId: graceLease.tenantId, leaseId: graceLease.id, amount: new Decimal(25000), mpesaReference: ref, accountNumber: 'KAM-KILELESHWA-A1', phoneFrom: '+254733111222', provider: PaymentProvider.MPESA, matchStatus: PaymentMatchStatus.MATCHED, paidAt: new Date(charge.billingPeriod.getFullYear(), charge.billingPeriod.getMonth(), 3) },
      });
      await prisma.paymentAllocation.create({ data: { paymentId: payment.id, rentChargeId: charge.id, amountApplied: new Decimal(25000) } });
      await prisma.rentCharge.update({ where: { id: charge.id }, data: { amountPaid: new Decimal(25000), balance: new Decimal(0), status: ChargeStatus.PAID } });
